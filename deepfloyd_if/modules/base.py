...
class IFBaseModule:
    ...
    def __init__(self, dir_or_name, device, pil_img_size=256, cache_dir=None, hf_token=None, filename=None):
    ...
    def embeddings_to_image(
        ...
        **kwargs,
        ...
        bs_scale = 1 if positive_t5_embs is None else 3  # was 2 if positive_t5_embs is 1, avoid forced CFG
        ...
        if support_noise is None:
            ...
            diffusion.support_noise_less_qsample_steps = 0
        else:
            assert support_noise_less_qsample_steps < len(diffusion.timestep_map) - 1
            # assert support_noise.shape == (1, 3, image_h, image_w)
            diffusion.support_noise_less_qsample_steps = support_noise_less_qsample_steps
        ...
        if sample_loop == 'ddpm':
            with torch.no_grad():
                sample = diffusion.p_sample_loop(
                    model_fn,
                    (batch_size * bs_scale, self.inCh, image_h, image_w),
                    noise=noise,
                    clip_denoised=True,
    ...
    def load_checkpoint(self, model, dir_or_name, filename='pytorch_model.bin'):
        param_device = 'cpu'
        if filename is not None and filename.endswith('.pt'):
            checkpoint = torch.load(filename, map_location='cpu')
            print('loading supplied checkpoint, NOT base IF, epoch: ',
                  checkpoint['epoch'])
            for param_name, param in checkpoint['model_state_dict'].items():
                try:
                    set_module_tensor_to_device(
                        model, param_name, param_device, value=param)
                except:
                    print('param_name state dict error: ', param_name)
        ...
        if os.path.exists(path):
            ...
            for param_name, param in checkpoint.items():
                try:
                    ...
                    print('failed to copy in the following tensor: ',
                          param_name, param.shape)
                    if not param_name.startswith('encoder_p'):
                        tmp_tens = torch.zeros(
                            [param.shape[0], self.inCh, 3, 3], dtype=self.model.dtype)
                        _ = torch.nn.init.xavier_uniform_(tmp_tens)
                        set_module_tensor_to_device(
                            model, param_name, param_device, value=tmp_tens)

        else:
            print(
                f'Warning! In directory "{dir_or_name}" filename "pytorch_model.bin" is not found.')

        if model.control_model is not None:
            for param_name, m in model.control_model.input_hint_block.named_parameters():
                tmp_tens = torch.zeros(m.shape, dtype=self.model.dtype)
                if len(m.shape) > 1:
                    _ = torch.nn.init.xavier_uniform_(tmp_tens)
                set_module_tensor_to_device(
                    model, 'control_model.input_hint_block.' + param_name, param_device, value=tmp_tens)

            for param_name, m in model.control_model.middle_block_out.named_parameters():
                tmp_tens = torch.zeros(m.shape, dtype=self.model.dtype)
                if len(m.shape) > 1:
                    _ = torch.nn.init.xavier_uniform_(tmp_tens)
                set_module_tensor_to_device(
                    model, 'control_model.middle_block_out.' + param_name, param_device, value=tmp_tens)

            for param_name, m in model.control_model.named_parameters():
                try:
                    param_val = checkpoint.get(param_name)
                    set_module_tensor_to_device(
                        model, 'control_model.' + param_name, param_device, value=param_val)
                except:
                    pass

            for param_name, m in model.control_model.zero_convs.named_parameters():
                tmp_tens = torch.zeros(m.shape, dtype=self.model.dtype)
                # zero convs must be zero
                set_module_tensor_to_device(
                    model, 'control_model.zero_convs.' + param_name, param_device, value=tmp_tens)

            for param_name, m in model.named_parameters():
                if m.is_meta:
                    assert (param_name == 'control_model.input_blocks.0.0.weight')
                    tmp_tens = torch.zeros(m.shape, dtype=self.model.dtype)
                    if len(m.shape) > 1:
                        _ = torch.nn.init.xavier_uniform_(tmp_tens)
                    set_module_tensor_to_device(
