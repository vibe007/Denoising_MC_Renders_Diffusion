...
class UNetModel(nn.Module):
    ...
    def __init__(
            ...
            conv_resample=True,
            dims=2,
            makeZeroConv=False, #for ControlNet
            num_classes=None,
            precision='32',
            num_heads=1,
            num_head_channels=-1,
        ...
        self.efficient_activation = efficient_activation
        self.scale_skip_connection = scale_skip_connection
        if makeZeroConv:  # makeZeroConv indicates this module is for CN
...
class ControlledUnetModel(UNetModel):
    def forward(self, x, timesteps=None, text_emb=None, control=None, timestep_text_emb=None, aug_emb=None, use_cache=False, only_mid_control=False, **kwargs):
        hs = []
        emb = self.time_embed(timestep_embedding(
            timesteps, self.model_channels, dtype=self.dtype))
        if use_cache and self.cache is not None:
            encoder_out, encoder_pool = self.cache
        else:

            text_emb = text_emb.type(self.dtype)  # [8, 77, 4096]
            encoder_out = self.encoder_proj(text_emb)
            encoder_out = encoder_out.permute(0, 2, 1)  # NLC -> NCL
            if timestep_text_emb is None:
                timestep_text_emb = text_emb
...
class ControlNet(UNetModel):
    """
    Define the ControlNet architecture, which is just the U-Net Encoder
    """

    def __init__(self, aux_ch=None, *args, **kwargs):
        super().__init__(makeZeroConv=True, aux_ch=aux_ch, *args, **kwargs)
        del self.output_blocks  # we don't need the output blocks
        del self.out
        dims = 2
        self.input_hint_block = TimestepEmbedSequential(
            conv_nd(dims, self.aux_ch, 48, 3, padding=1),
            nn.SiLU(),
            conv_nd(dims, 48, 48, 3, padding=1),
            nn.SiLU(),
            conv_nd(dims, 48, 64, 3, padding=1, stride=1),
            nn.SiLU(),
            conv_nd(dims, 64, 64, 3, padding=1),
            nn.SiLU(),
            conv_nd(dims, 64, 96, 3, padding=1, stride=1),
            nn.SiLU(),
            conv_nd(dims, 96, 96, 3, padding=1),
            nn.SiLU(),
            conv_nd(dims, 96, 128, 3, padding=1, stride=1),
            nn.SiLU(),
            zero_module(conv_nd(dims, 128, self.model_channels, 3, padding=1))
        )

    def forward(self, x, timesteps, text_emb, hint, timestep_text_emb=None, aug_emb=None, use_cache=False, **kwargs):
        outs = []
        emb = self.time_embed(timestep_embedding(
            timesteps, self.model_channels, dtype=self.dtype))
        if use_cache and self.cache is not None:
        ...
        guided_hint = self.input_hint_block(hint, emb, encoder_out)
        x = guided_hint
        h = x.type(self.dtype)
...
class SuperResUNetModel(ControlledUnetModel):
    """
    ...
    def __init__(self, low_res_diffusion, interpolate_mode='bilinear', *args, **kwargs):
        ...
        if self.doCN:
            self.control_model = ControlNet(*args, **kwargs)
            print('Creating ControlNet...')
        else:
            self.control_model = None

    def forward(self, x, timesteps, low_res,hint=None, aug_level=None, **kwargs):
        ...
        if self.doCN:
            control = self.control_model(
                x=None, hint=hint, timesteps=timesteps, aug_emb=aug_emb, **kwargs)
        else:
            control = None
        return super().forward(x, timesteps, control=control, aug_emb=aug_emb, **kwargs)