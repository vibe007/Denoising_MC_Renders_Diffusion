...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.super27_steps = [0, 100, 200, 300, 400, 500, 600, 700, 799, 800, 825, 850, 874, 899, 900, 
          908, 916, 925, 933, 941, 950, 958, 966, 974, 982, 991, 999]
    ...
    def p_sample(
        ...
        if inpainting_mask is None:
            inpainting_mask = torch.ones_like(x[:,0:3,...], device=x.device)

        sample = out['mean'] + nonzero_mask * torch.exp(0.5 * out['log_variance']) * noise
        sample = (1 - inpainting_mask)*x[:,0:3,...] + inpainting_mask*sample
    ...
    def p_sample_loop_progressive(
        ...
        indices = indices[self.support_noise_less_qsample_steps:]
        if progress:
        ...
        for i in indices:
            ...
            with torch.no_grad():
                ...
                yield out
                img = out['sample']
                if shape[1] > img.shape[1]: #check if the shape of img is different from model kwargs
                    img = torch.cat([img,out['feat']],dim=1)
    ...
    def training_losses(self, model, x_start, feats=None, model_kwargs=None, noise=None, bias_tsteps=False):
        """
        ...
        """
        def _efficient_biased_sampler(batch_size, device):           
            # Generate a random mask for choosing between t_list and full range
            mask = np.random.random(batch_size) < 0.5
            
            # Initialize the result array
            result = np.empty(batch_size, dtype=np.int64)
            
            # Fill in values from t_list where mask is True
            result[mask] = np.random.choice(self.super27_steps, size=mask.sum())
            
            # Fill in values from full range where mask is False
            result[~mask] = np.random.randint(0, 1000, size=(~mask).sum())
            
            # Convert to int64 torch tensor
            return torch.from_numpy(result).to(device=device, dtype=torch.int64)

        if bias_tsteps:
            t = _efficient_biased_sampler(x_start.shape[0],x_start.device)
        else:
            t = torch.randint(low = 0, high=1000, size=[x_start.shape[0]]).to(x_start.device)
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = torch.randn_like(x_start)
